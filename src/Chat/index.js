...
const Chat = () => {
  ...
  const handleDownloadHistory = () => {
    // Combine chat and chatHistory into a single array if needed
    const combinedHistory = [...chatHistory, ...chat];

    // Convert chat history to a string (JSON.stringify, or any format you prefer)
    const chatHistoryString = JSON.stringify(combinedHistory, null, 2);

    // Create a Blob from the string
    const blob = new Blob([chatHistoryString], { type: 'application/json' });
        ...
        <div className=' h-[20%]'>
          <div className=' flex flex-col items-center justify-center w-full h-full text-white'>
            <div className=' w-[60%] flex justify-center relative'>
              ...
              <span className=' absolute right-4 top-4 cursor-pointer' onClick={() => input.trim() ? handleTest() : undefined}>
